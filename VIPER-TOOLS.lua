...
raknet = require("lib.samp.raknet")
 function update()
    local raw = "https://raw.githubusercontent.com/zongrg/Vasdosao941Tools/main/AutoUpdate.json"
    local dlstatus = require('moonloader').download_status
    ...
    function f:getLastVersion()
        ...
        if response.status_code == 200 then
            return decodeJson(response.text)['2026']
        else
            return 'UNKNOWN'
        end
    end
...
colors = imgui.CreateTextureFromFile(getGameDirectory() .. "\\moonloader\\AdminViper\\color.png")
authors = imgui.CreateTextureFromFile(getGameDirectory() .. "\\moonloader\\AdminViper\\authors.png")
...
--		nopf5 = true,
		...
		nead = 60,
		mdm = 5,
		sendgoodplay = tostring(u8'/ao Администрация Arizona RP Viper желает Вам приятной игры на просторах нашего сервера!'),
		cmdHouse = 'ohome',
		cmdBusiness = 'obiz',
...
local telegrampiar = imgui.ImBuffer(1024)
...
local selector_list = {
fa.ICON_FA_USER_COG..u8' Настройки',
fa.ICON_FA_PALETTE..u8' Настройки цветов',
fa.ICON_FA_KEYBOARD..u8' бинды',
fa.ICON_FA_BAN..u8' Менеджер наказаний',
...
fa.ICON_FA_CAMERA..u8' Слёт меню',
fa.ICON_FA_SKULL..u8' Bullet Tracing',
fa.ICON_FA_INFO_CIRCLE..u8' Информация о скрипте'
}
...
'Теперь конфиг скрипта хранится в папке скрипта, а не в папке config для всего мунлоадера (путь: moonloader/config/AdminViper/AdminTools.ini) '
		},
		['patches'] = {
		['show'] = false,
			['info'] = {
...
local function createNewJsonTable(table)
	if not doesFileExist(getWorkingDirectory().."\\AdminViper\\"..table[2]..".json") then
	   	local f = io.open(getWorkingDirectory().."\\AdminViper\\"..table[2]..".json", 'w+')
	   	if f then
		  	f:write(encodeJson(table[1])):close()
		end
   	else
	   	local f = io.open(getWorkingDirectory().."\\AdminViper\\"..table[2]..".json", "r")
	   	if f then
...
pathOffLog = getWorkingDirectory().."\\AdminViper\\Логи\\AutoOpraLogs.txt"
log_full = {}
...
--	-- дальше идёт ваш код
 ...
 ImgForAuthor = "https://raw.githubusercontent.com/zongrg/Vasdosao941Tools/main/Viper.jpg"

function LoadTextureFunc()
	textureImgForAuthor = imgui.CreateTextureFromFile(getWorkingDirectory() .. '/AdminViper/Viper.jpg')
end
function main()
	...
	-- Если надо привязать сервер к айпи
	--[[local server, port = sampGetCurrentServerAddress();
	while server ~= "IP" do wait(15);
		sampAddChatMessage("TOOLS Viper DOES NOT WORK ON THIS SERVER, ONLY ON VIPER: IP", -1);
		sampAddChatMessage("SCRIPT DEVELOPER: Viper - Вадим Зонгов, LAST - FAKE", -1);
	end]]

	if not doesFileExist(getWorkingDirectory() .. '/AdminViper/Viper.jpg') then
		downloadUrlToFile(ImgForAuthor, getWorkingDirectory() .. '/AdminViper/Viper.jpg', function (id, status, p1, p2)
			if status == dlstatus.STATUSEX_ENDDOWNLOAD then
				LoadTextureFunc()
				sampAddChatMessage('vk.com/arz_viper t.me/arz_viper', -1)
			end
		end)
	else
		LoadTextureFunc()
		  ...
		  if tonumber(param) ~= tonumber(cfg.Settings.mode) then
			for k, v in pairs(messageChangeMode) do
			  if (param + 1) == k then
				...
				cfg.Settings.mode = (k - 1); inicfg.save(cfg, "AdminViper\\AdminTools.ini"); break
			  end
			end
		  else
...
--		end
	...
	if cfg.main.gm then
		...
		if cfg.main.airbrake then 
			...
			printStringNow("~w~Tools Viper "..(enAirBrake and "~b~OFF" or "~g~ON"), 2000)
		...
		if isKeysDown(facceptform.v) and not sampIsChatInputActive() and not sampIsDialogActive() and not isPauseMenuActive() and not isSampfuncsConsoleActive() and cfg.main.trueform then
			...
			lua_thread.create(function()
				...
				local f = io.open(getGameDirectory().."//moonloader//AdminViper//AcceptForms.txt", 'a')
				...
				autoformaq = "false"
				wait (502)
				sampSendChat("/a [Viper-TOOLS] ~ Forma [+]")
				wait (502)
				sampAddChatMessage("{00BFFF}[Viper-TOOLS] - Успешно принятая форма от: " .. admin_nick .. "{FFFFFF}")	
				acceptforma = "true"
			end)
			end
		end
...
function imgui.BeforeDrawFrame()
	...
	if fsClock == nil then
        ...
        -- Сама логика переключения фокуса
        if main_window_state.v then
            -- Если зажата Правая Кнопка Мыши (VK_RBUTTON)
            if isKeyDown(key.VK_RBUTTON) then
                imgui.Process = false -- Отключаем обработку ImGui (курсор пропадает, камера вертится)
            else
                imgui.Process = true  -- Включаем обратно, когда отпустили
            end
        else
            imgui.Process = false
        end
...
local X, Y = getScreenResolution()
	...
	if MainWindow.v then
		...
		imgui.Begin(fa.ICON_FA_TOOLS..u8' ARIZONA VIPER | Tools '..fa.ICON_FA_TOOLS, MainWindow, imgui.WindowFlags.NoResize + imgui.WindowFlags.NoSavedSettings) 	
	   ...
	   if  imgui.Button(fa.ICON_FA_SAVE..u8'', imgui.ImVec2(54, 27)) then
			...
			inicfg.save(cfg, "AdminViper\\AdminTools.ini")
	   end
...
imgui.Text(u8'Задержка между отправкой строк:')
			...
			if imgui.Button(u8'Сохранить и закрыть', imgui.ImVec2(-1, 25)) then
				local file = io.open(getGameDirectory().."//moonloader//AdminViper//zawitabinder.txt", "w")
		...
		if imgui.BeginPopupModal(fa.ICON_FA_COGS..u8' Настройки | Множитель доната', MainWindow, imgui.WindowFlags.NoCollapse + imgui.WindowFlags.NoResize + imgui.WindowFlags.NoMove + imgui.WindowFlags.NoSavedSettings + imgui.WindowFlags.AlwaysAutoResize) then
			...
			if imgui.Button(u8'Сохранить и закрыть', imgui.ImVec2(-1, 25)) then
				local file = io.open(getGameDirectory().."//moonloader//AdminViper//donatebinder.txt", "w")
...
imgui.InputTextMultiline('##kotofqee1v16noyob', rulesbinder, imgui.ImVec2(650, 250))
			...
			if imgui.Button(u8'Сохранить и закрыть', imgui.ImVec2(-1, 25)) then
				local file = io.open(getGameDirectory().."//moonloader//AdminViper//rulesbinder.txt", "w")
			...
			lua_thread.create(function ()
				for cikk22 in telegrampiar.v:gmatch("[^\r\n]+") do
					sampSendChat(u8:decode(cikk22))
...
imgui.InputTextMultiline('##eweqewqewdas', telegrampiar, imgui.ImVec2(650, 250))
			imgui.Text(u8'Задержка между отправкой строк:')
			imgui.SameLine()
			imgui.PushItemWidth(100)
			...
			if imgui.Button(u8'Сохранить и закрыть', imgui.ImVec2(-1, 25)) then
				local file = io.open(getGameDirectory().."//moonloader//AdminViper//telegrampiar.txt", "w")
				file:write(telegrampiar.v)
				file:close()
				imgui.CloseCurrentPopup()
			end
				imgui.EndPopup()
...
imgui.InputTextMultiline('##eweqquuewqewdas', groupbinder, imgui.ImVec2(650, 250))
			...
			if imgui.Button(u8'Сохранить и закрыть', imgui.ImVec2(-1, 25)) then
				local file = io.open(getGameDirectory().."//moonloader//AdminViper//groupbinder.txt", "w")
		...
		if imgui.Button(u8'Собственный №1', imgui.ImVec2(200, 22)) then 
			...
			if imgui.Button(u8'Сохранить и закрыть', imgui.ImVec2(-1, 25)) then
				local file = io.open(getGameDirectory().."//moonloader//AdminViper//binder1.txt", "w")
...
imgui.InputTextMultiline('##kotofeevnoob', mybinder2, imgui.ImVec2(650, 250))
			...
			if imgui.Button(u8'Сохранить и закрыть', imgui.ImVec2(-1, 25)) then
				local file = io.open(getGameDirectory().."//moonloader//AdminViper//binder2.txt", "w")
...
imgui.Text(u8'Задержка между отправкой строк:')
			...
			if imgui.Button(u8'Сохранить и закрыть', imgui.ImVec2(-1, 25)) then
				local file = io.open(getGameDirectory().."//moonloader//AdminViper//binder3.txt", "w")
...
imgui.Text(u8'Задержка между отправкой строк:')
			...
			if imgui.Button(u8'Сохранить и закрыть', imgui.ImVec2(-1, 25)) then
				local file = io.open(getGameDirectory().."//moonloader//AdminViper//binder4.txt", "w")
...
imgui.Separator()
	...
	if imgui.Button(u8("Добавить кнопку ответа в репорт"), imgui.ImVec2(-1, 25)) then
		...
		if menusel.v == 13 then
			...
			imgui.PopFont()
			imgui.PushFont(fs80)
			imgui.SetCursorPosX((imgui.GetWindowWidth() - imgui.CalcTextSize('Viper CODER').x) / 2)
			imgui.TextColored(imgui.ImVec4(rainbowtext(4)), 'Viper CODER')
			imgui.PopFont()
			imgui.PushFont(fs16)
			imgui.CenterTextColoredRGB('Реализовано для проекта:')
			imgui.SetCursorPosX((imgui.GetWindowWidth() - imgui.CalcTextSize('ARIZONA RP Viper').x) / 2 - 65)
			imgui.TextColored(imgui.ImVec4(rainbowtext(4)), 'ARIZONA RP | vk.com/arz_viper')
			imgui.PopFont()
			imgui.PushFont(fs16)
						...
						sampSetCursorMode(CMODE_LOCKCAM)
							...
							if isKeyDown(VK_LBUTTON) then 
								...
								for k,v in pairs(adminsJ_) do 
									if doesFileExist(getWorkingDirectory().."\\AdminViper\\"..adminsJ_[k][2]..".json") then 
										local f = io.open(getWorkingDirectory().."\\AdminViper\\"..adminsJ_[k][2]..".json", "w+") 
										if f then 
						...
						sampSetCursorMode(CMODE_LOCKCAM)
							...
							if isKeyDown(VK_LBUTTON) then 
								...
								for k,v in pairs(J_) do 
									if doesFileExist(getWorkingDirectory().."\\AdminViper\\"..J_[k][2]..".json") then 
										local f = io.open(getWorkingDirectory().."\\AdminViper\\"..J_[k][2]..".json", "w+") 
										if f then 
...
if ReportWindow.v then 
		...
		imgui.Begin(u8'Жалоба/Вопрос', ReportWindow, imgui.WindowFlags.NoCollapse + imgui.WindowFlags.NoResize + imgui.WindowFlags.NoMove + imgui.WindowFlags.NoSavedSettings + imgui.WindowFlags.AlwaysAutoResize)
			...
			if not doesFileExist(getGameDirectory()..'//moonloader//AdminViper//gps.json') then
			...
			imgui.OpenPopup(u8'Помощь по GPS')
				...
				local gps = io.open('moonloader\\AdminViper\\gps.json', 'a+')
	...
	gps:close()
		...
		if imgui.Button(v, imgui.ImVec2(510, 25)) then
			...
			if not doesFileExist(getGameDirectory()..'//moonloader//AdminViper//commands.json') then
			...
			imgui.OpenPopup(u8'Список команд')
				...
				local commands = io.open('moonloader\\AdminViper\\commands.json', 'a+')
...
local f = io.open(getGameDirectory().."//moonloader//AdminViper//Логи//UnjailsLogs.txt", 'a')
...
local f = io.open(getGameDirectory().."//moonloader//AdminViper//Логи//UnmutesLogs.txt", 'a')
...
local f = io.open(getGameDirectory().."//moonloader//AdminViper//Логи//UnbanLogs.txt", 'a')
...
local f = io.open(getGameDirectory().."//moonloader//AdminViper//Логи//UnbanipLogs.txt", 'a')
...
local f = io.open(getGameDirectory().."//moonloader//AdminViper//Логи//BanoffLogs.txt", 'a')
...
local f = io.open(getGameDirectory().."//moonloader//AdminViper//Логи//JailoffLogs.txt", 'a')
...
local f = io.open(getGameDirectory().."//moonloader//AdminViper//Логи//MuteoffLogs.txt", 'a')
...
local f = io.open(getGameDirectory().."//moonloader//AdminViper//Логи//BanipoffLogs.txt", 'a')
...
if not isGamePaused() then	
	...
	if text:find("(.*) %[(.*)%] купил бизнес ID: (.*) по гос. цене за (.*) ms! Капча: %((%d+) | (%d+)%)") and cfg.autoopra.settingbiz then
		...
		local f = io.open(getGameDirectory().."\\moonloader\\AdminViper\\Логи\\AutoOpraLogs.txt", 'a')
...
if not doesDirectoryExist(getGameDirectory()..'//moonloader//AdminViper') then 
	createDirectory(getGameDirectory()..'//moonloader//AdminViper') 
	sampAddChatMessage(tag..'Директория для нужных файлов скрипта успешно создана', -1)
end
if not doesDirectoryExist(getGameDirectory()..'//moonloader//AdminViper//Логи//') then 
	createDirectory(getGameDirectory()..'//moonloader//AdminViper//Логи//') 
	sampAddChatMessage(tag..'Директория для логов успешно создана', -1)
end
	if not doesFileExist(getGameDirectory().."//moonloader//AdminViper//binder1.txt") then
		local f = io.open(getGameDirectory().."//moonloader//AdminViper//binder1.txt", 'w')
		f:close()
		sampAddChatMessage(tag..'Файл для биндера "Собственный №1" успешно создан', -1)
	end
	if not doesFileExist(getGameDirectory().."//moonloader//AdminViper//binder2.txt") then
		local f = io.open(getGameDirectory().."//moonloader//AdminViper//binder2.txt", 'w')
		f:close()
		sampAddChatMessage(tag..'Файл для биндера "Собственный №2" успешно создан', -1)
	end
	
	if not doesFileExist(getGameDirectory().."//moonloader//AdminViper//binder3.txt") then
		local f = io.open(getGameDirectory().."//moonloader//AdminViper//binder3.txt", 'w')
		f:close()
		sampAddChatMessage(tag..'Файл для биндера "Собственный №3" успешно создан', -1)
	end
	
	if not doesFileExist(getGameDirectory().."//moonloader//AdminViper//binder4.txt") then
		local f = io.open(getGameDirectory().."//moonloader//AdminViper//binder4.txt", 'w')
		f:close()
		sampAddChatMessage(tag..'Файл для биндера "Собственный №4" успешно создан', -1)
	end
	
	if not doesFileExist(getGameDirectory().."//moonloader//AdminViper//AcceptForms.txt") then
		local f = io.open(getGameDirectory().."//moonloader//AdminViper//AcceptForms.txt", 'w')
		f:close()
		sampAddChatMessage(tag..'Файл для записи принятых форм успешно создан', -1)
	end
	
	if not doesFileExist(getGameDirectory().."//moonloader//AdminViper//GiveWarnsTK.txt") then
		local f = io.open(getGameDirectory().."//moonloader//AdminViper//GiveWarnsTK.txt", 'w')
		f:close()
		sampAddChatMessage(tag..'Файл для записи выданных варнов за ТК успешно создан', -1)
	end
	
	if not doesFileExist(getGameDirectory().."//moonloader//AdminViper//zawitabinder.txt") then
		local f = io.open(getGameDirectory().."//moonloader//AdminViper//zawitabinder.txt", 'w')
	...
	if not doesFileExist(getGameDirectory().."//moonloader//AdminViper//donatebinder.txt") then
		local f = io.open(getGameDirectory().."//moonloader//AdminViper//donatebinder.txt", 'w')
	...
	if not doesFileExist(getGameDirectory().."//moonloader//AdminViper//rulesbinder.txt") then
		local f = io.open(getGameDirectory().."//moonloader//AdminViper//rulesbinder.txt", 'w')
	...
	if not doesFileExist(getGameDirectory().."//moonloader//AdminViper//telegrampiar.txt") then
		local f = io.open(getGameDirectory().."//moonloader//AdminViper//telegrampiar.txt", 'w')
	...
	if not doesFileExist(getGameDirectory().."//moonloader//AdminViper//groupbinder.txt") then
		local f = io.open(getGameDirectory().."//moonloader//AdminViper//groupbinder.txt", 'w')
		f:write(u8'/ao Уважаемые игроки, хотите узнавать актуальную информацию самым первым?\n/ao Для такого случая у нас есть паблик ВК - vk.com/arz_viper, узнавайте новости самым первым!')
		f:close()
		sampAddChatMessage(tag..'Файл для биндера "Паблик VK" успешно создан', -1)
	end
	
	if not doesFileExist(getGameDirectory().."//moonloader//AdminViper//Логи//AdminVipChatLogs.txt") then
		local f = io.open(getGameDirectory().."//moonloader//AdminViper//Логи//AdminVipChatLogs.txt", 'w')
		f:close()
		sampAddChatMessage(tag..'Файл для записи сообщений от администраторов в VIP чате создан.', -1)
	end
	if not doesFileExist(getGameDirectory().."//moonloader//AdminViper//Логи//UnjailsLogs.txt") then
		local f = io.open(getGameDirectory().."//moonloader//AdminViper//Логи//UnjailsLogs.txt", 'w')
		f:close()
		sampAddChatMessage(tag..'Файл для записи выпусков из деморгана создан.', -1)
	end
	if not doesFileExist(getGameDirectory().."//moonloader//AdminViper//Логи//UnmutesLogs.txt") then
		local f = io.open(getGameDirectory().."//moonloader//AdminViper//Логи//UnmutesLogs.txt", 'w')
		f:close()
		sampAddChatMessage(tag..'Файл для записи снятий мутов создан.', -1)
	end
	if not doesFileExist(getGameDirectory().."//moonloader//AdminViper//Логи//UnbanLogs.txt") then
		local f = io.open(getGameDirectory().."//moonloader//AdminViper//Логи//UnbanLogs.txt", 'w')
		f:close()
		sampAddChatMessage(tag..'Файл для записи разблокировок создан.', -1)
	end
	if not doesFileExist(getGameDirectory().."//moonloader//AdminViper//Логи//UnbanipLogs.txt") then
		local f = io.open(getGameDirectory().."//moonloader//AdminViper//Логи//UnbanipLogs.txt", 'w')
		f:close()
		sampAddChatMessage(tag..'Файл для записи разблокировок IP создан.', -1)
	end
	if not doesFileExist(getGameDirectory().."//moonloader//AdminViper//Логи//BanoffLogs.txt") then
		local f = io.open(getGameDirectory().."//moonloader//AdminViper//Логи//BanoffLogs.txt", 'w')
		f:close()
		sampAddChatMessage(tag..'Файл для записи блокировок в оффлайне создан.', -1)
	end
	if not doesFileExist(getGameDirectory().."//moonloader//AdminViper//Логи//JailoffLogs.txt") then
		local f = io.open(getGameDirectory().."//moonloader//AdminViper//Логи//JailoffLogs.txt", 'w')
		f:close()
		sampAddChatMessage(tag..'Файл для записи выдачи деморганов в оффлайне создан.', -1)
	end
	if not doesFileExist(getGameDirectory().."//moonloader//AdminViper//Логи//MuteoffLogs.txt") then
		local f = io.open(getGameDirectory().."//moonloader//AdminViper//Логи//MuteoffLogs.txt", 'w')
		f:close()
		sampAddChatMessage(tag..'Файл для записи выдачи мутов в оффлайне создан.', -1)
	end
	if not doesFileExist(getGameDirectory().."//moonloader//AdminViper//Логи//BanipoffLogs.txt") then
		local f = io.open(getGameDirectory().."//moonloader//AdminViper//Логи//BanipoffLogs.txt", 'w')
		f:close()
		sampAddChatMessage(tag..'Файл для записи выдачи блокировок IP в оффлайне создан.', -1)
	end
	
	if not doesFileExist(getWorkingDirectory().."\\AdminViper\\Логи\\AutoOpraLogs.txt") then
		local f = io.open(getGameDirectory().."//moonloader//AdminViper//Логи//AutoOpraLogs.txt", 'w')
		f:close()
		sampAddChatMessage(tag..'Файл для записи покупок имущества создан.', -1)
	end
	
	mybind1 = io.open(getGameDirectory().."//moonloader//AdminViper//binder1.txt", "r")
	mybinder1.v = mybind1:read('*a')
	mybind1:close()
	
	mybind2 = io.open(getGameDirectory().."//moonloader//AdminViper//binder2.txt", "r")
	mybinder2.v = mybind2:read('*a')
	mybind2:close()
	
	mybind3 = io.open(getGameDirectory().."//moonloader//AdminViper//binder3.txt", "r")
	mybinder3.v = mybind3:read('*a')
	mybind3:close()
	
	mybind4 = io.open(getGameDirectory().."//moonloader//AdminViper//binder4.txt", "r")
	mybinder4.v = mybind4:read('*a')
	mybind4:close()
	
	zawitabind = io.open(getGameDirectory().."//moonloader//AdminViper//zawitabinder.txt", "r")
	zawitabinder.v = zawitabind:read('*a')
	zawitabind:close()
	
	donatebind = io.open(getGameDirectory().."//moonloader//AdminViper//donatebinder.txt", "r")
	donatebinder.v = donatebind:read('*a')
	donatebind:close()
	
	rulesbind = io.open(getGameDirectory().."//moonloader//AdminViper//rulesbinder.txt", "r")
	rulesbinder.v = rulesbind:read('*a')
	rulesbind:close()
	
	discordbind = io.open(getGameDirectory().."//moonloader//AdminViper//telegrampiar.txt", "r")
	telegrampiar.v = discordbind:read('*a')
	discordbind:close()
	
	groupbind = io.open(getGameDirectory().."//moonloader//AdminViper//groupbinder.txt", "r")
...
IMAGES = {
	KILL_LIST = {
	[0] = "moonloader/AdminViper/KillList/weap_0.png",
	[1] = "moonloader/AdminViper/KillList/weap_1.png",
	[2] = "moonloader/AdminViper/KillList/weap_2.png",
	[3] = "moonloader/AdminViper/KillList/weap_3.png",
	[4] = "moonloader/AdminViper/KillList/weap_4.png",
	[5] = "moonloader/AdminViper/KillList/weap_5.png",
	[6] = "moonloader/AdminViper/KillList/weap_6.png",
	[7] = "moonloader/AdminViper/KillList/weap_7.png",
	[8] = "moonloader/AdminViper/KillList/weap_8.png",
	[9] = "moonloader/AdminViper/KillList/weap_9.png",
	[10] = "moonloader/AdminViper/KillList/weap_10.png",
	[11] = "moonloader/AdminViper/KillList/weap_11.png",
	[12] = "moonloader/AdminViper/KillList/weap_12.png",
	[13] = "moonloader/AdminViper/KillList/weap_13.png",
	[14] = "moonloader/AdminViper/KillList/weap_14.png",
	[15] = "moonloader/AdminViper/KillList/weap_15.png",
	[16] = "moonloader/AdminViper/KillList/weap_16.png",
	[17] = "moonloader/AdminViper/KillList/weap_17.png",
	[18] = "moonloader/AdminViper/KillList/weap_18.png",
	[19] = "moonloader/AdminViper/KillList/weap_19.png",
	[20] = "moonloader/AdminViper/KillList/weap_20.png",
	[21] = "moonloader/AdminViper/KillList/weap_21.png",
	[22] = "moonloader/AdminViper/KillList/weap_22.png",
	[23] = "moonloader/AdminViper/KillList/weap_23.png",
	[24] = "moonloader/AdminViper/KillList/weap_24.png",
	[25] = "moonloader/AdminViper/KillList/weap_25.png",
	[26] = "moonloader/AdminViper/KillList/weap_26.png",
	[27] = "moonloader/AdminViper/KillList/weap_27.png",
	[28] = "moonloader/AdminViper/KillList/weap_28.png",
	[29] = "moonloader/AdminViper/KillList/weap_29.png",
	[30] = "moonloader/AdminViper/KillList/weap_30.png",
	[31] = "moonloader/AdminViper/KillList/weap_31.png",
	[32] = "moonloader/AdminViper/KillList/weap_32.png",
	[33] = "moonloader/AdminViper/KillList/weap_33.png",
	[34] = "moonloader/AdminViper/KillList/weap_34.png",
	[35] = "moonloader/AdminViper/KillList/weap_35.png",
	[36] = "moonloader/AdminViper/KillList/weap_36.png",
	[37] = "moonloader/AdminViper/KillList/weap_37.png",
	[38] = "moonloader/AdminViper/KillList/weap_38.png",
	[39] = "moonloader/AdminViper/KillList/weap_39.png",
	[40] = "moonloader/AdminViper/KillList/weap_40.png",
	[41] = "moonloader/AdminViper/KillList/weap_41.png",
	[42] = "moonloader/AdminViper/KillList/weap_42.png",
	[43] = "moonloader/AdminViper/KillList/weap_43.png",
	[44] = "moonloader/AdminViper/KillList/weap_44.png",
	[45] = "moonloader/AdminViper/KillList/weap_45.png",
	[46] = "moonloader/AdminViper/KillList/weap_46.png",
	[49] = "moonloader/AdminViper/KillList/weap_49.png",
	[50] = "moonloader/AdminViper/KillList/weap_50.png",
	[51] = "moonloader/AdminViper/KillList/weap_51.png",
	[53] = "moonloader/AdminViper/KillList/weap_53.png",
	[54] = "moonloader/AdminViper/KillList/weap_54.png",
	[255] = "moonloader/AdminViper/KillList/weap_255.png"
...
sampAddChatMessage(tag..'Разработчик: {FF69B4}Viper', -1)
sampAddChatMessage(tag..'Этот скрипт работает только на: {FF69B4}ARIZONA RP VIPER', -1)
end
